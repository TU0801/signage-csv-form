// セキュリティ: HTMLエスケープ（グローバルスクリプトのため独自定義。ES Module版は shared-utils.js）
...
// テンプレート画像辞書・getTemplateImageUrl・hasTemplateImage は template-images.js に分離

// 管理者用: 選択中のベンダーID（管理者が他社として入力する場合）
let selectedVendorIdForAdmin = null;

// 紐づけ（ベンダー×点検種別）された点検種別の実DB id 集合。
// 空でない＝「紐づけ文脈あり」とみなし、点検種別の並べ替え（紐づけを上）・先頭自動選択・
// 案内カテゴリの初期選択を有効化する。紐づけ文脈が無いとき（管理者でベンダー未選択など）は
// 従来どおり全件・未選択のまま振る舞う。
let linkedInspectionIds = new Set();

// 管理者がベンダー選択を高速に切り替えた際の非同期競合を防ぐ世代トークン。
// onAdminVendorChange は呼び出しごとに値を増やし、await 後に自分が最新かを確認して
// 最新でなければ共有状態(linkedInspectionIds)や画面への反映を中止する（last-wins）。
let adminVendorChangeSeq = 0;
        ...
        let entries = [];
        window._entries = entries; // csv-generator.js から参照
        let editingIndex = -1;
        ...
        async function init() {
            ...
            // 一般ユーザー（ベンダー）ログイン時は、そのベンダーの紐づけを反映して
            // カテゴリ初期選択・点検種別の並べ替え・先頭自動選択を適用する。
            // 管理者（currentVendorId なし）はベンダー選択時に onAdminVendorChange 側で適用。
            if (window.currentVendorId) {
                linkedInspectionIds = await loadLinkedInspections(window.currentVendorId);
                applyLinkedDefaults();
            } else {
        ...
        function populateInspectionTypeSelect(category = '') {
            ...
            // 現在の選択値を保持（明示的に選んだ値は再描画後も維持する）
            const currentValue = select.value;
            select.innerHTML = '<option value="">選択してください</option>';

            // カテゴリフィルター: カテゴリが空（全て）なら全件。
            // 実カテゴリ列(category)を持つ項目はそれで完全一致判定する。旧実装は点検種別「名」への
            // 部分一致だったため、名前に「点検」を含まない点検カテゴリ項目（例: 植栽の手入れ・貯水槽清掃・
            // 各種調査）が漏れていた。実カテゴリ列で判定して修正する。
            // 実カテゴリを持たない項目（Supabase取得失敗時のハードコードfallback等のレガシーデータ）は
            // 従来どおり名前の部分一致にフォールバックして、絞り込みが空になる退行を防ぐ。
            const entries = [];
            masterData.notices.forEach((n, i) => {
                const matches = !category
                    || (n.category ? n.category === category : n.inspectionType.includes(category));
                if (matches) {
                    entries.push({ n, i });
                }
            });

            // 紐づけ済みの点検種別を上に、それ以外を下に（安定ソートでマスター並び順は各グループ内で維持）
            if (linkedInspectionIds.size > 0) {
                entries.sort((a, b) => {
                    const la = linkedInspectionIds.has(a.n.inspectionId) ? 0 : 1;
                    const lb = linkedInspectionIds.has(b.n.inspectionId) ? 0 : 1;
                    return la - lb;
                });
            }
            ...
            entries.forEach(({ n, i }) => {
                const opt = document.createElement('option');
                opt.value = i;                       // 値は元のnotices配列index（onInspectionTypeChangeで参照）
            ...
            } else if (linkedInspectionIds.size > 0 && entries.length > 0) {
                // 紐づけ文脈では先頭（＝最上位の紐づけ項目）を初期選択
                select.value = String(entries[0].i);
            }
        }
        ...
        function onCategoryChange() {
            ...
            // 再描画後の選択状態（紐づけ文脈なら先頭自動選択、それ以外は未選択）をプレビュー等へ反映
        ...
        // 指定ベンダーに紐づけ（active）された点検種別の実DB idの集合を返す（純関数・グローバルは変更しない）。
        // 取得失敗時は空集合を返して従来挙動へフォールバックする（致命的にしない）。
        // 呼び出し側が結果を linkedInspectionIds へ反映することで、非同期の競合（last-wins）を制御できる。
        async function loadLinkedInspections(vendorId) {
            const ids = new Set();
            if (!vendorId || typeof window.getVendorInspections !== 'function') return ids;
            try {
                const rels = await window.getVendorInspections(vendorId);
                (rels || []).forEach(r => {
                    if (r && r.inspection_id) ids.add(r.inspection_id);
                });
            } catch (e) {
                console.error('紐づけ点検種別の取得に失敗しました:', e);
            }
            return ids;
        }

        // 紐づけ文脈に基づく初期表示を適用する。
        // Q1: 紐づけされた点検種別が「すべて同一の、かつ案内カテゴリに選択可能な1カテゴリ」のときだけ、
        //     その案内カテゴリを初期選択する。null/未分類や、その他のようにUIドロップダウンに無いカテゴリが
        //     混在・単独で存在する場合は、紐づけ項目を隠さないため「全て」のままにする。
        // Q2/Q3: 点検種別リストは紐づけを上に（マスター表示順で）並べ、先頭を初期選択する
        //        （populateInspectionTypeSelect 内で実施。「先頭」＝マスター表示順で最初の紐づけ項目）。
        function applyLinkedDefaults() {
            const catSelect = document.getElementById('inspectionCategory');
            // null/未分類は '(none)' という番兵で別扱いにし、1カテゴリへ誤って潰さない
            const linkedCats = new Set(
                masterData.notices
                    .filter(n => linkedInspectionIds.has(n.inspectionId))
                    .map(n => n.category || '(none)')
            );
            if (linkedCats.size === 1) {
                const onlyCat = [...linkedCats][0];
                // ドロップダウンに存在する実カテゴリのときだけ初期選択（'(none)'やUI未掲載カテゴリは対象外）
                if (onlyCat !== '(none)' && [...catSelect.options].some(o => o.value === onlyCat)) {
                    catSelect.value = onlyCat;
                }
            }
            populateInspectionTypeSelect(catSelect.value);
            onInspectionTypeChange();
        }
        ...
        function updatePreview() {
            ...
            if (imgUrl) {
                ...
                // 画像読み込み後にコンテナをフィットさせ、フォントサイズ用CSS変数を設定
                const img = container.querySelector('img');
                const fitContainer = () => {
                    if (!img.naturalWidth || !img.naturalHeight) return;
                    const ratio = img.naturalWidth / img.naturalHeight;
                    // 親要素の幅と60vhから実際の表示サイズを計算
                    const parentWidth = container.parentElement.clientWidth;
                    const maxH = window.innerHeight * 0.6;
                    const heightFromWidth = parentWidth / ratio;
                    let w, h;
                    ...
                    container.style.setProperty('--pw', w);
                };
                ...
                window._posterResizeHandler = () => fitContainer();
        ...
        // CSV生成・ダウンロード・プレビューは csv-generator.js に分離
        ...
        async function onAdminVendorChange() {
            ...
            // この呼び出しの世代。await 後に自分が最新でなければ反映を中止する（last-wins）
            const mySeq = ++adminVendorChangeSeq;
            ...
            if (!vendorId) {
                ...
                // 紐づけ文脈をクリアし、点検種別リストを全件・未選択に戻す。
                // 先に選択値を空へ落とすことで、再描画時に直前の明示選択が復元されるのを防ぐ。
                linkedInspectionIds = new Set();
                document.getElementById('inspectionCategory').value = '';
            ...
            if (vendorIndex !== -1) {
                ...
                // 点検種別: 選択ベンダーの紐づけ（ベンダー×点検種別）を読み込み、
                // 紐づけを上に並べ替え＋案内カテゴリ初期選択（1種類のみ）＋先頭自動選択を適用。
                // 旧実装はベンダーの単一 inspection_type 文字列で1件だけ自動選択しロックしていたが、
                // 複数紐づけに対応できないため廃止し、編集可能なまま紐づけ反映に置き換える。
                const inspectionSelect = document.getElementById('inspectionType');
                inspectionSelect.disabled = false;
                inspectionSelect.style.background = '';
                const ids = await loadLinkedInspections(vendorId);
                if (mySeq !== adminVendorChangeSeq) return; // 後続の選択に追い越された → 反映中止
                linkedInspectionIds = ids;
                applyLinkedDefaults();
            }
        ...
        // downloadCSV, copyCSV, previewCSV は csv-generator.js で公開
        ...
        // closeModal は csv-generator.js で公開
