...
### 9. バージョン管理の忘れ（NEW - 2026-01-03）

#### #9-1: プッシュ時にバージョンを上げ忘れる
- **発生回数**: 1回（今回、7回連続で忘れた）
- **症状**: どのバージョンに何が含まれているか分からなくなる
- **原因**: バージョンアップをプッシュ後に実施していた
- **対策**:
  ```bash
  # プッシュ前に必ず実施
  1. version.js を更新（v1.20.X → v1.20.X+1）
  2. git add js/version.js
  3. git commit -m "chore: Bump version to vX.XX.XX"
  4. git push
  ```
- **チェックリスト**:
  ```
...
### 10. テスト失敗の放置（NEW - 2026-01-03） ⭐️ 最重要

#### #10-1: CI/Playwrightテストが失敗しても放置
- **発生回数**: 1回（今回、Playwright 15件失敗を放置）
- **症状**: CI が赤でもプッシュを続ける、新機能を追加し続ける
- **原因**: テスト修正を後回しにした、優先度を間違えた
- **対策**:
  ```bash
  # プッシュ前に必ず実行
  npx playwright test
...
- **チェックリスト**:
  ...
  ```
- **重要度**: CRITICAL（品質の基盤）
- **教訓**: テストが赤 = プロダクトが壊れている

---

### 11. UI/UX を一発で完成させない（NEW - 2026-01-03）

#### #11-1: 繰り返し修正でfix率が上がる
- **発生回数**: 1回（今回、モーダルUIで4回修正）
- **症状**: ユーザーから「使いづらい」「おかしい」と指摘され、繰り返し修正
- **原因**: 実装前にUI/UXを十分に検討していない、ユーザーに確認していない
- **対策**:
  ```
  1. 実装前にワイヤーフレーム/デザインを提示
  2. ユーザーに確認・承認を得る
  3. プロフェッショナルなUIガイドラインに従う
  4. 実装後、自分でブラウザテスト
  5. スクリーンショットをユーザーに確認
  6. OKをもらってからプッシュ
  ```
